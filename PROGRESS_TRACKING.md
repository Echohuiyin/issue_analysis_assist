...
## V2.6 优先目标刷新（2026-03-08）

### 新优先级
- P0：第一部分「案例获取与解析」与第二部分「案例存储与向量化」联调闭环优先完成。
- P1：其余模块（SKILL训练、自动分析）在上述闭环稳定后继续推进。

### 第一部分（案例获取与解析）联调策略
- 多源抓取：`StackOverflow`、`CSDN`、`Zhihu` 统一接入，支持 API + HTML fallback。
- 本地小模型解析：默认 `Ollama + Qwen(1.8B)`，根据机器资源保留 Mock/规则回退。
- 提示词结构：标题、模块、现象、分析过程、关联代码、根因、解决措施、修复代码。
- 质量门槛：`quality_score < 80` 直接丢弃，不进入存储链路。

### 第二部分（案例存储与向量化）联调策略
- 数据库存储：Django 支持 `SQLite / PostgreSQL` 双模式，使用环境变量切换。
- CRUD验证：新增联调脚本覆盖增删改查。
- 本地RAG向量库：案例入库后自动向量化（轻量本地实现）并支持相似检索。
